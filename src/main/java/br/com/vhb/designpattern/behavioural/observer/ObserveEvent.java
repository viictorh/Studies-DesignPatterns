...
class EventObs<TArgs> {
	...
	public class Subscription implements AutoCloseable {
		private EventObs<TArgs> event;
		private int id;

		public Subscription(EventObs<TArgs> event, int id) {
			this.event = event;
			this.id = id;
		}
...
class Person {
	public EventObs<PropertyChangedEventArgs> propertyChanged = new EventObs<>();
...
public class ObserveEvent {
	public static void main(String[] args) {
		...
		EventObs<PropertyChangedEventArgs>.Subscription sub = person.propertyChanged.addHandler(x -> {
